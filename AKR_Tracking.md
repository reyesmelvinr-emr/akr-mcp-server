...
- D11 normalization dependency: D11-B (template normalization) and D11-F (distribution and downstream repo sync) are BLOCKING prerequisites for Deliverable 3 Mode B runs. No Mode B pilot run on CourseDomain, EnrollmentDomain, or UserDomain may begin until D11-B and D11-F are marked COMPLETE. Rationale: SSG timing data (pass-timings-seconds, total-generation-seconds, premium-requests) collected before template normalization carries an uncontrolled variable — approximately 2,800 tokens of removed template prose inflating Pass 1 load. Pre-normalization timing cannot be used as the Phase 2.5 strategy decision baseline in benchmark.json. If D11-B and D11-F cannot be completed before Mode B begins, each pre-normalization Mode B run must be flagged [PRE-NORMALIZATION — EXCLUDE FROM BENCHMARK] in the SSG timing data collection table.
- D11-A charter reduction dependency: D11-A (charter normalization) is NOT blocking for Mode B pilot start but MUST be complete before Phase 2 retrospective sign-off. Rationale: charter-load-path-error-rate (the observable metric for whether the correct instructions file was loaded vs. the full charter) cannot be meaningfully baseline-captured until the full charter has been restructured such that loading the wrong artifact produces a detectable token-count difference in generation metadata.
